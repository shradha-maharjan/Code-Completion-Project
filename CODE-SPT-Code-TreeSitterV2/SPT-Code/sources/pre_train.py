...
def pre_train(args,
    ...
    for task in tasks:
        ...
        if task == enums.TASK_CODE_AST_PREDICTION:
            logger.info('-' * 100)
            if args.n_epoch_pre_train_cap != 30:
                print(f'n_epoch_pre_train is updated to {args.n_epoch_pre_train_cap}')
            logger.info('-' * 100)
            model.set_model_mode(enums.MODEL_MODE_CLS)
            logger.info('-' * 100)
            training_args = TrainingArguments(output_dir=os.path.join(args.pre_train_output_root, task),
                                              ...
                                              weight_decay=args.lr_decay_rate,
                                              max_grad_norm=args.grad_clipping_norm,
                                              num_train_epochs=args.n_epoch_pre_train,#n_epoch_pre_train_cap,
                                              lr_scheduler_type=SchedulerType.LINEAR,
                                              warmup_steps=args.warmup_steps,
                                              logging_dir=os.path.join(args.tensor_board_root, task),
                                              logging_strategy=IntervalStrategy.STEPS,
            ...
            logger.info('-' * 100)
            if args.n_epoch_pre_train_mass != 30:
                print(f'n_epoch_pre_train_mass is updated to {args.n_epoch_pre_train_mass}')
            # set model mode
            logger.info('-' * 100)
            ...
            training_args = Seq2SeqTrainingArguments(output_dir=os.path.join(args.pre_train_output_root, task),
                                                     ...
                                                     weight_decay=args.lr_decay_rate,
                                                     max_grad_norm=args.grad_clipping_norm,
                                                     num_train_epochs=args.n_epoch_pre_train, #n_epoch_pre_train_mass, # updated
                                                     lr_scheduler_type=SchedulerType.LINEAR,
                                                     warmup_steps=args.warmup_steps,
                                                     logging_dir=os.path.join(args.tensor_board_root, task),
                                                     logging_strategy=IntervalStrategy.STEPS,
