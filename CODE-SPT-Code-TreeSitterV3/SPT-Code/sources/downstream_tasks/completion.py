...
def run_completion(
    ...
    #     logger.info('Calling trainer.predict() for test dataset')
        
    #     small_test_dataset = torch.utils.data.Subset(datasets['test'], range(10)) 
    ...
    #     logger.info(f'Predict metrics: {predict_metrics}')
        
    #     references = predict_metrics.pop('test_references', None)
    #     candidates = predict_metrics.pop('test_candidates', None)
        
    #     if references is None or candidates is None:
    #         logger.warning('References or candidates are missing from predict_metrics')
        
    #     trainer.log_metrics(split='test', metrics=predict_metrics)
    #     trainer.save_metrics(split='test', metrics=predict_metrics)
        
    #     logger.info('Starting to save testing results to files')
    #     with open(os.path.join(args.output_root, f'{enums.TASK_COMPLETION}_test_results.txt'), mode='w', encoding='utf-8') as result_f, \
    #         open(os.path.join(args.output_root, f'{enums.TASK_COMPLETION}_test_refs.txt'), mode='w', encoding='utf-8') as refs_f, \
    #         open(os.path.join(args.output_root, f'{enums.TASK_COMPLETION}_test_cans.txt'), mode='w', encoding='utf-8') as cans_f:
            
    #         logger.info('Writing predictions to files...')
    #         sample_id = 0
    #         for reference, candidate in zip(references, candidates):
    #             logger.debug(f'Writing sample {sample_id} to files')
    ...
    #         logger.info(f'Total samples written: {sample_id}')
            
    #         for name, score in predict_metrics.items():
    #             result_f.write(f'{name}: {score}\n')
    #     logger.info('Testing results saved successfully.')
        
    # except Exception as e:
    #     logger.error(f"An error occurred during prediction phase: {e}", exc_info=True)
    # #     logger.error(f"An error occurred during prediction phase: {e}", exc_info=True)
    trainer.compute_metrics = compute_test_metrics
