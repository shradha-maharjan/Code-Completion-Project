...
#nohup python main.py --do-pre-train --pre-train-tasks cap,mass --batch-size 16 --eval-batch-size 32 --cuda-visible-devices 0 --fp16 --model-name pre_train --n-epoch 1 --n-epoch-pre-train 1 --pre-train-subset-ratio 1 --parse-subset-ratio 0.1 --task completion --remove-existing-saved-file pre_train:fine_tune > output-spt-code-2024-$DATE.log 2>&1 &
nohup python main.py --do-pre-train --pre-train-tasks cap --batch-size 96 --eval-batch-size 96 --cuda-visible-devices 0,1,2 --fp16 --model-name pre_train --n-epoch 30 --task completion --remove-existing-saved-file pre_train:fine_tune > output-spt-code-2024-$DATE.log 2>&1 &
