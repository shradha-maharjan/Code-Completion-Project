...
class CodeDataset(Dataset):
    ...
    def get_sample_ratio(self, dataset_size):
        if dataset_size == 'small':
            return 0.25  # Sample 25% of the data
        elif dataset_size == 'mid':
            return 0.50  # Sample 50% of the data
        elif dataset_size == 'large':
            return 1.0  # No sampling (use all data)
        else:
            raise ValueError("Invalid dataset size argument. Must be 'small', 'mid', or 'large'.")
...
def init_dataset(args, mode, task=None, language=None, split=None, clone_mapping=None,
    ...
    if load_if_saved:
        ...
        path_org = os.path.join(args.dataset_save_dir, f'{name}_org.pk')
