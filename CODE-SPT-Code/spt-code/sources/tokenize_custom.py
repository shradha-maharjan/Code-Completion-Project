...
def tokenize_source(source, lang):
    """Tokenize source code based on language, skipping [MSK] tokens and reassembling multi-character operators."""
    if lang not in MAPPING_LANG_LEXER:
        logger.error(f"Lexer for language '{lang}' not found.")
        return ""
    ...
    for i, part in enumerate(parts):
        if part.strip():
            ...
            lexer_class = MAPPING_LANG_LEXER[lang]
            lexer = lexer_class(input_stream)
            tokens.extend(token.text for token in lexer.getAllTokens())
        if i < len(parts) - 1:
            tokens.append("[MSK]")
    
    tokens = reassemble_tokens(tokens)
    
    # Join tokens into a single line
    code = ' '.join(tokens)
    #code = replace_string_literal(' '.join(tokens))
    return trim_spaces(code)

def read_and_tokenize_file(input_file_path, tokenized_output_file_path, lang = "java"):
    with open(input_file_path, 'r') as infile, open(tokenized_output_file_path, 'w') as tokenized_outfile:
        for idx, line in enumerate(tqdm(infile, desc="Tokenizing lines")):
            try:
                # Tokenize the current line
                tokenized_line = tokenize_source(line.strip(), lang=lang)
                # Write the tokenized line to the output file
                tokenized_outfile.write(tokenized_line + '\n')
            except Exception as e:
                logger.warning(f"Failed to tokenize line {idx}: {e}")
