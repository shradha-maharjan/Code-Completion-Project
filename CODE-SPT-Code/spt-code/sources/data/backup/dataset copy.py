...
from .data_utils import load_dataset_from_dir, set_args, \
    ...
    parse_for_bug_fix, handle_error
        ...
        with open('mismatched_sources.txt', 'w') as file:
            for i, (file_source, file_ast) in enumerate(zip(sources_from_file, asts_from_file)):
                if i < len(self.sources):  # Check if index is within the bounds of self.sources
                    ...
                    normalized_file_source = self.remove_whitespaces(self.lowercase(file_source))
                    normalized_self_source = self.remove_whitespaces(self.lowercase(self_source))
                    ...
                    if not self.compare(normalized_file_source, normalized_self_source):
    ...
    def __init__(self, args, dataset_name, mode, task=None, language=None, split=None, clone_mapping=None):
        ...
        if self.mode == 'pre_train':
            ...
            if args.ast_type == "jdt":
                self.jdt_file_path = '/home/user1-selab3/Documents/research-shradha/CODE-SPT-Code/spt-code/sources/output_file.txt'
                print("JDT flag is set, loading ASTs from:", self.jdt_file_path)
                # Unpacking both sources and ASTs returned by the method
                sources_from_file, self.asts = self.load_ast_from_file_jdt(self.jdt_file_path)

                self.compare_and_save_sources(sources_from_file, self.asts)
