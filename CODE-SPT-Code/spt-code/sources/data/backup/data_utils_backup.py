    ...
    for file in os.listdir(dataset_dir):
        ...
        if len(dataset_files) > 0:
            ...
            for dataset_file_path in dataset_files:
                ...
                ast_file_path = os.path.join(ast_directory, f"{lang}_ast_trees.jsonl")
                jdt_file_path = os.path.join(jdt_directory, f"jdt_java_exampleFunction.jsonl") 
                ...
                with open(ast_file_path, 'w') as write_file:
                    for source, code, name, code_wo_name in tqdm(zip(sources, codes, names, codes_wo_name),
                        ...
                        try:
                            ...
                            #         ast = load_ast_from_file_jdt(file=jdt_file_path)
                            #     elif main_args.ast_type == "tree-sitter":
                            #         with open(ast_file_path, 'r') as read_file:
                            #             asts = [json.loads(line.strip()) for line in read_file]
                                ...
                                if main_args.ast_type == "jdt":
                                   ...
                                   only_names.append(name)
                                    ...
                                    with open(ast_file_path, 'r') as f:
                                        for line in f:
                                            asts.append(json.loads(line.strip()))
                                    new_sources.append(source)
                                    new_codes.append(code)
                                    new_codes_wo_name.append(code_wo_name)
                                    new_names.append(nl)
