    ...
    for file in os.listdir(dataset_dir):
        ...
        if len(dataset_files) > 0:
            ...
            for dataset_file_path in dataset_files:
                ...
                process_dataset_files(dataset_file_path, sources, codes, names, codes_wo_name, docs, lang, main_args,
                                  new_sources, new_codes, new_codes_wo_name, new_names, new_names_wo_name, only_names, asts,languages)

            logger.info(f'  {lang} dataset size: {n_sample}')

    assert len(languages) == len(all_sources) == len(all_asts)  # Ensure data consistency
    return paths, languages, all_sources, all_codes, all_asts, all_names, all_codes_wo_name, all_names_wo_name, all_only_names, all_docs

def process_dataset_files(dataset_file_path, sources, codes, names, codes_wo_name, docs, lang, main_args,
                          new_sources, new_codes, new_codes_wo_name, new_names, new_names_wo_name, only_names, asts,languages):
    if main_args.ast_save and main_args.ast_type == "tree-sitter":
        save_asts(dataset_file_path, sources, names, lang)

    if main_args.ast_load:
        ast = load_asts(dataset_file_path, lang, main_args.ast_type)
        asts.append(ast)  # Append loaded ASTs to the main list

    # Process other data
    new_sources.append(sources)
    new_codes.append(codes)
    new_codes_wo_name.append(codes_wo_name)
    new_names.append([generate_single_ast_nl(name) for name in names])  # Assuming generate_nl function exists
    new_names_wo_name.append([generate_single_ast_nl(name, replace_method_name=True) for name in names])  # Handle names without method names
...
def save_asts(dataset_file_path, sources, names, lang):
    ast_file_path = os.path.join('/home/user1-selab3/Documents/research-shradha/CODE-SPT-Code/spt-code/sources/data/asts/ast_saved2', f"{lang}_ast_tree.jsonl")
    os.makedirs(os.path.dirname(ast_file_path), exist_ok=True)
    with open(ast_file_path, 'w') as write_file:
        for source, name in zip(sources, names):
            ast, nl, nl_wo_name = generate_single_ast_nl(source, lang, name, True)
            write_file.write(json.dumps({'ast': ast, 'nl': nl, 'nl_wo_name': nl_wo_name}) + "\n")

def load_asts(dataset_file_path, lang, ast_type):
    ast_file_path = os.path.join('/home/user1-selab3/Documents/research-shradha/CODE-SPT-Code/spt-code/sources/data/asts/ast_saved2', f"{lang}_ast_tree.jsonl")
    jdt_file_path = os.path.join('/home/user1-selab3/Documents/research-shradha/CODE-SPT-Code/spt-code/sources/data/asts/jdt_saved2', f"{lang}_ast_tree.jsonl")
    asts = []
    if main_args.ast_type == "tree-sitter":
        with open(ast_file_path, 'r') as read_file:
            for line in read_file:
                ast = json.loads(line.strip())
                asts.append(ast)
    else:
        with open(jdt_file_path, 'r') as read_file:
            for line in read_file:
                ast = json.loads(line.strip())
                asts.append(ast)
