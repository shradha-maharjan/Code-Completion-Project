...
def load_dataset_from_dir(dataset_dir):
    ...
    all_sources = []
    all_asts = []
    jdt_asts = load_ast_from_file_jdt(jdt_file_path)
    all_codes = []
    all_codes_wo_name = []
    all_names = []
    all_names_wo_name = []
                ...
                for idx, (source, code, name, code_wo_name) in enumerate(tqdm(zip(sources, codes, names, codes_wo_name),
                    ...
                    try:
                        if main_args.ast_type == "jdt":
                            ...
                            ast = jdt_asts[idx] #retrieves the AST at position idx in the jdt_asts list and this AST is then assigned to the variable ast.
                            ...
                            only_names.append(name)
                            asts.append(ast)
                        # all_asts.append(jdt_asts[idx])  # Append the corresponding AST
