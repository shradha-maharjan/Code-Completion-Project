...
def iter_pre_train_dataset_files(lang_dir, lang):
    ...
    if lang in [enums.LANG_GO, enums.LANG_JAVA, enums.LANG_PYTHON, enums.LANG_JAVASCRIPT, enums.LANG_PHP,
                enums.LANG_RUBY]:
        return [file for file in iter_all_files(base=lang_dir) if file.endswith('.jsonl')]
        # files = [file for file in iter_all_files(base=lang_dir) if file.endswith('.jsonl')]
        # files.sort(key=lambda x: os.path.getsize(x))  # Sort by file size
        # return files
    return []
...
def load_dataset_from_dir(dataset_dir, lang=None):
    """
    Load all files in the given dir, only for pre-training, filtered by a specific language if provided.

    Args:
        dataset_dir (str): Root directory containing language directories.
        lang (str, optional): Language to filter. If None, all languages will be considered.
    ...
    if lang is None:
        logger.error("No language specified. Please provide a language to filter the dataset.")
        sys.exit()

    path = os.path.join(dataset_dir, lang)
    if not os.path.isdir(path):
        logger.error(f'Language directory "{lang}" not found in {dataset_dir}.')
        sys.exit()
...
def parse_for_completion(source_path, target_path):
    ...
    Args:
        ...
        ast_path (str): Path of ASTs (required if ast_type is 'jdt')
        nl_path (str): Path of NLs (required if ast_type is 'jdt')
